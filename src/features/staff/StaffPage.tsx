...
export const StaffPage: React.FC = () => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setFormLoading(true);
    try {
      await staffService.createStaff(organisationId, authUser.uid, {
        firstName,
        lastName,
        email,
        role
  ...
  const handleArchive = async (id: string) => {
    ...
    if (confirm('Are you sure you want to archive this staff member?')) {
      try {
        await staffService.archiveStaff(organisationId, authUser.uid, id);
        window.location.reload();
      } catch (err) {
        console.error(err);
            ...
            <tbody>
              {filteredStaff.map(s => (
                <tr key={s.id} className="border-b border-slate-100 hover:bg-slate-50">
                  <td className="p-4 font-medium text-slate-800">{s.firstName} {s.lastName}</td>
                  ...
                  <td className="p-4 text-right flex justify-end gap-2">
                    ...
                    <button onClick={() => handleArchive(s.id)} className="text-slate-400 hover:text-red-600 p-1"><Trash2 className="w-4 h-4" /></button>
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                ...
                <select required className="w-full border border-gray-300 rounded p-2" value={role} onChange={e => setRole(e.target.value)}>
